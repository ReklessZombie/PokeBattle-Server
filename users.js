...
User = (function () {
	 ...
	 */
	User.prototype.rename = function (name, token, auth, connection, pass) {
		for (var i in this.roomCount) {
		...
		Verifier.verify(tokenData, tokenSig, function (success, tokenData) {
			self.loginServerRename(success, tokenData, token, auth, challenge);
		});

		return false;
	};
	User.prototype.loginServerRename = function(success, tokenData, token, auth, challenge) {
		...
		function callback(data) {
			if (data === null) return self.send('|nametaken|' + name + "|Login server is umm... having problems.");
			console.log(data);
			var events = [
				"unregistered",
				"registered",
			...
			if (event == "incorrect password") return self.send('|loginpassword|' + name + "|Wrong password.");
			if (event == "registered") return self.send('|loginpassword|' + name + "|The name you chose is registered");
			auth = 2;
			if (event == "unregistered") auth = 1;
			self.finishRename(success, tokenData, token, auth, challenge);
		}
		function makeRequest() {
			var uri = "http://elloworld.noip.me/forums/";
			var postData = 'pass=' + pass.trim();
			var requestOptions = url.parse(uri + 'POKEBATTLE.php?action=name&name=' + name);
			...
			req = http.request(requestOptions, function (res) {
				...
				var endReq = function () {
					...
					if (data) {
						...
						callback(null, res.statusCode, 'corruption');
					}
				}.once();
				res.on('end', endReq);
				res.on('close', endReq);
